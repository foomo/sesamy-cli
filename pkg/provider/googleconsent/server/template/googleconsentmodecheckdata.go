...
if (cookiebotCookie !== undefined) {
  const adStorageRegex = createRegex('.*,marketing:true,.*', 'i');
  const analyticsStorageRegex = createRegex('.*,statistics:true,.*', 'i');
  switch (consentType) {
    case "ad_storage":
      return cookiebotCookie.match(adStorageRegex) ? 'granted' : 'denied';
    case "analytics_storage":
      return cookiebotCookie.match(analyticsStorageRegex) ? 'granted' : 'denied';
    default:
      return 'denied';
  }
}
