...
const MixpanelTagData = `___INFO___
  ...
  },
  {
    ...
    "subParams": [
      ...
      },
      {
        "type": "TEXT",
        "name": "utmAttribution",
        "displayName": "UTM Attribution",
        "simpleValueType": true,
        "help": "Bind the {{UTM Attribution}} variable here to share first-touch/last-touch UTM values across tags instead of re-parsing the URL in each one."
      },
      {
        "type": "SIMPLE_TABLE",
...
let cookieOptions = {
  domain: '.'+getRequestHeader('x-original-forwarded-host') || getRequestHeader('x-forwarded-host') || getRequestHeader('host'),
  path: '/',
  samesite: 'Lax',
  secure: true,
...
function setDistinctIdCookies(distinctId, deviceId) {
  if (!getCookieValues('stape_mixpanel_distinct_id')[0]) setCookie('stape_mixpanel_distinct_id', makeString(distinctId), cookieOptions);
  if (!getCookieValues('stape_mixpanel_device_id')[0]) setCookie('stape_mixpanel_device_id', makeString(deviceId), cookieOptions);
}
...
function setSimplifiedIdCookies(deviceId, userId) {
  if (!getCookieValues('stape_mixpanel_device_id')[0]) setCookie('stape_mixpanel_device_id', makeString(deviceId), cookieOptions);
  if (userId && !getCookieValues('stape_mixpanel_user_id')[0]) setCookie('stape_mixpanel_user_id', makeString(userId), cookieOptions);
}
...
function trackCommonData(postBody) {
  ...
  // Fall back to the referer header for server-only events with no page_referrer
  // (e.g. a purchase API call that never carried the browser's document.referrer).
  const referrer = eventData.page_referrer || getRequestHeader('referer');
  if (referrer) postBody.properties['$referrer'] = referrer;
  if (referrer) postBody.properties['$referring_domain'] = parseUrl(referrer).hostname;

  const url = eventData.page_location || getRequestHeader('referer');
  const urlParsed = parseUrl(url);

  // --------------------------------------------------------------------------
  // UTM handling — delegated to the shared "UTM Attribution" GTM variable
  // (utm-attribution-variable.js) fed by the "Attribution Cookie Writer" tag
  // (utm-attribution-writer-tag.js). Both persistence (first-touch cookie)
  // and parsing now live outside this tag, so every other tag in the
  // container (GA4, Meta CAPI, ...) shares the same values instead of each
  // re-implementing its own cookie logic.
  //
  // Bind the {{UTM Attribution}} variable to this tag's "utmAttribution"
  // field in the Template Editor's field configuration.
  ...
  utmKeys.forEach((utmKey) => {
    const initialKey = 'initial_' + utmKey;

    if (utmAttribution[utmKey]) postBody.properties[utmKey] = utmAttribution[utmKey];

    // Respect explicit opt-out via the "Remove Parameters" table, same as $initial_referrer.
    const initialRemoved =
      data.trackParametersRemove && data.trackParametersRemove.some((el) => el.name === initialKey);
    if (!initialRemoved && utmAttribution[initialKey]) {
      postBody.properties[initialKey] = utmAttribution[initialKey];
    }
    // Absent in both cases ⇒ property omitted, matching Mixpanel's "(not set)" convention.
  });

  // These extended UTM parameters aren't part of the shared Attribution
  // variable (source_platform/campaign_id/etc. are rarely needed for
  // cross-tag first-touch persistence) — parsed directly from this event's
  // own URL as before.
  if (urlParsed) {
